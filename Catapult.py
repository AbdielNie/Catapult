...
class Catapult():
    ...
    def draw(self):
        ...
        pygame.draw.circle(screen, LIGHTBLUE,
                           (int(self.dx * 360 / self.xrange) + 47, 427 - int(self.dy * 280 / self.yrange)), self.radius)
...
creditsLabel = font.render("For Physics Unit1 project", 1, WHITE)
...
while application:
    ...
    if not simulating:
        if simulateButton.update() and v0Box.text != "" and angleBox.text != "" and heightBox.text != "":
            xrangeoutput.text = (
            "{:.4f}".format(get_xrange(float(heightBox.text), float(v0Box.text), float(angleBox.text) / 57.2958)))
